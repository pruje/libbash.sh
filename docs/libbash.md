...
- `$lb_yes_mode`: (boolean, false by default) if set to `true`, it will answer "yes" in every call of `lb_yesno` and `lbg_yesno`
...
Available functions:
- `lb_display_critical` (or `lb_critical`)
- `lb_display_error` (or `lb_err`)
- `lb_display_warning` (or `lb_warning`, or `lb_warn`)
...
--strict             Force user to prompt yes or no (disables default option)
